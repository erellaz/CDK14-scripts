...
webhook=SyncWebhook.from_url(r"https://discord.com/api/webhooks/0000/xxxxxxxx")
...
def CleanUp(pathinput,df,selector,verbosity,webhook):
    ...
    Talk(verbosity,"Removing files due to bad Eccentricity: "+str(df.loc[df['Eccentricity']>selector['Eccentricity']].shape[0]),webhook)
    MoveToTrash(df.loc[df['Eccentricity']>selector['Eccentricity'],["FilePath"]],trashpath)
...
if os.path.exists(pathinput):
    ...
    try:
        for basename in os.listdir(rootpath):
            EXTENTIONS = (".json", ".csv")
            if basename.endswith(EXTENTIONS):
